...
# Final safe version of get_annual_stats
get_annual_stats <- function(df, var, group_col = "Year") {
  df %>%
...
# Function to compute annual stats
get_annual_stats <- function(df, var, group_col = "Year") {
  df %>%
...
# Whole year summaries
delt <- get_annual_stats(cursp, "Delma.MA", "Year")
ras <- get_annual_stats(cursp, "RasGhanada.MA", "Year")
saa <- get_annual_stats(cursp, "Saadiyat.MA", "Year")
ala <- get_annual_stats(cursp, "AlAqah.MA", "Year")
dib <- get_annual_stats(cursp, "Dibba.MA", "Year")

# Merge temp1 data to cursplong
cursplong1 <- merge(cursplong, temp1[, c("date1", "Site", "yearAD", "CMA_max", "CMA_min")], by = c("date1", "Site"), all.x=TRUE)
cursplong1$winteryear <- ifelse(cursplong1$month == "DEC" & cursplong1$CMA_min == 1, cursplong1$Year + 1, cursplong1$Year)

# Winter and Summer subsets
wcursp <- subset(cursplong1, CMA_min == 1)
scursp <- subset(cursplong1, CMA_max == 1)

# Function to get seasonal summaries
get_season_summary <- function(df, season_col, site_name, season_prefix) {
  res <- ddply(df, .(get(season_col), Site == site_name), summarise,
               maxM = max(MA), minM = min(MA), avM = mean(MA), sdM = sd(MA),
               maxR = max(raw), minR = min(raw), avR = mean(raw), sdR = sd(raw))
  names(res)[2] <- "TF"
  res <- subset(res, TF == TRUE)[, -2]
  res$site <- site_name
  colnames(res)[1] <- "year"
  colnames(res)[2:9] <- paste0(season_prefix, colnames(res)[2:9])
  return(res)
}

# Generate seasonal summaries
Wcssumall <- do.call(rbind, lapply(sites, function(s) get_season_summary(wcursp, "winteryear", s, "Wcs")))
Scssumall <- do.call(rbind, lapply(sites, function(s) get_season_summary(scursp, "Year", s, "Scs")))
...
test <- test[,-grep(pattern="\\.x|\\.y", colnames(test))]

# Add gulf variable
test$gulf <- ifelse(test$site %in% c("AlAqah", "Dibba"), "oman", "gulf")
