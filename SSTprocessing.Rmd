...
Here is a simple plot showing individual colony relationships between diameter and height
...
Now we can summarize our current speed variables in the coldest and hottest parts of each year, as we have done for SST. First we need to create a long-format version of the temperature data for parsing:

```{r, message=FALSE, warning=FALSE}
# Step 1: Add CMA_min and CMA_max indicators to templong
sumwin_df <- templong %>%
  group_by(Year, Site) %>%
  mutate(
    CMA_min = +(CMA == min(CMA, na.rm = TRUE)),
    CMA_max = +(CMA == max(CMA, na.rm = TRUE))
  ) %>%
  ungroup()

# Fix known data issue (you can skip this if 1985 isn't in your dataset)
sumwin_df[21, "CMA_min"] <- 0
sumwin_df[32, "CMA_min"] <- 1

# Step 2: Get rows for 30 days before and after CMA_min and CMA_max
extract_window <- function(df, flag_col) {
  inds <- which(df[[flag_col]] == 1)
  rows <- lapply(inds, function(x) (x - 30):(x + 30))
  selected <- df[unlist(rows), ]
  selected[[flag_col]] <- 1
  return(selected)
}
...
sumwin_sum <- extract_window(sumwin_df, "CMA_max")

# Step 3: Merge flags back into templong
temp1 <- templong %>%
  left_join(sumwin_sum %>% select(date1, Site, CMA_max), by = c("date1", "Site")) %>%
  left_join(sumwin_win %>% select(date1, Site, CMA_min), by = c("date1", "Site"))

# Ensure NAs in flags are 0
temp1$CMA_min[is.na(temp1$CMA_min)] <- 0
temp1$CMA_max[is.na(temp1$CMA_max)] <- 0

# Convert Site to factor and add winteryear
temp1$Site <- as.factor(temp1$Site)
temp1$winteryear <- ifelse(temp1$month == "DEC", temp1$Year + 1, temp1$Year)
```

Now we can get our current speed summaries:
```{r, message=FALSE, warning=FALSE}
site_vars <- c("Delma.MA", "RasGhanada.MA", "Saadiyat.MA", "AlAqah.MA", "Dibba.MA")

yearly_summaries <- purrr::map_dfr(site_vars, function(var) {
  cursp %>%
    group_by(Year) %>%
    summarise(
      csmaxM = max(.data[[var]], na.rm = TRUE),
      csminM = min(.data[[var]], na.rm = TRUE),
      csavM  = mean(.data[[var]], na.rm = TRUE),
      cssdM  = sd(.data[[var]], na.rm = TRUE),
      Site   = gsub(".MA", "", var)
    )
})

cursplong1 <- cursplong %>%
  left_join(temp1 %>% select(date1, Site, yearAD, CMA_max, CMA_min),
            by = c("date1", "Site")) %>%
  mutate(
    winteryear = if_else(month == "DEC" & CMA_min == 1, Year + 1, Year)
  )

wcursp <- filter(cursplong1, CMA_min == 1)
scursp <- filter(cursplong1, CMA_max == 1)

sites <- c("Delma", "RasGhanada", "Saadiyat", "AlAqah", "Dibba")

# Helper function
get_seasonal_summary <- function(df, group_var, site, prefix) {
  df %>%
    filter(Site == site) %>%
    ...
    rename_with(~ paste0(prefix, .), -c(year, Site))
}

# Run for all sites
Wcssumall <- purrr::map_dfr(sites, ~get_seasonal_summary(wcursp, "winteryear", .x, "Wcs"))
Scssumall <- purrr::map_dfr(sites, ~get_seasonal_summary(scursp, "Year", .x, "Scs"))
...
Scssumall <- Scssumall %>%
  mutate(AScsrangeM = ScsmaxM - ScsminM)

```


Now we finalise our dataset for analyses by joining all environmental variables to the coral growth data and making sure that there are complete cases for initial modeling of growth dataset:
```{r, message=FALSE, warning=FALSE}
...
Wtsumallm1 <- Wtsumall %>% mutate(year = year + 1)
Stsumallm1 <- Stsumall %>% mutate(year = year + 1)
wintrocfinm1 <- wintrocfin %>% mutate(year = year + 1)
summrocfinm1 <- summrocfin %>% mutate(year = year + 1)
Wcssumallm1 <- Wcssumall %>% mutate(year = year + 1)
Scssumallm1 <- Scssumall %>% mutate(year = year + 1)
